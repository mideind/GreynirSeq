...
class IndexedParallelBTDocumentsDataset(LanguagePairDataset):
    def __init__(
        ...
        self.flat_src = hf_datasets.Dataset.from_parquet("/data/scratch/haukur/document_translation/parquet/flat_src.parquet")
        ...
        self.flat_tgt = hf_datasets.Dataset.from_parquet("/data/scratch/haukur/document_translation/parquet/flat_tgt.parquet")

        self.bt_src_start = (
            np.cumsum([len(dset.flat_src) for dset in parallel_datasets])[-1]
            if parallel_datasets
            else 0
        )
        self.bt_tgt_start = (
            np.array([len(dset.flat_tgt) for dset in parallel_datasets])[-1]
            if parallel_datasets
            else 0
        )
        ...
        p_all_offsets = (
            np.concatenate(parallel_offsets) if parallel_offsets else empty_array
        )
    ...
    def __getitem__(self, index):
        ...
        maybe_noised_encode_fn = (
            self.encoder.encode_noisy if is_bt else self.encoder.encode
        )
    ...
    def interleave_indices(self):
        if self.epoch != self._interleave_seed:
            logger.info(
                f"Interleaving parallel and bt datasets with ratios={self.mixture_ratios}"
            )
            ...
            self.index_datset = hf_datasets.load_from_disk("/data/scratch/haukur/document_translation/parquet/index_dataset.foo")
