...
model = IcebertConstModel.from_pretrained(
    ...
    #data_name_or_path="/home/vesteinn/NER_PROCESSING/parice_data/parice_parsed_for_ner/eea/bin/input0"
    #data_name_or_path='/home/vesteinn/data/MIM-GOLD-NER/8_entity_types/bin/bin',
...
if USE_DATASET:
    ...
    for dataset_offset in range(dataset_size): #np.random.randint(0, dataset_size, 100):
        ...
        # 
        # eval_ner.compare(res, target_cats)
...
#infile = open('/data/datasets/en-is/v4/train/en-is.train.is')
#infile = open('en-is.train.is.rest')
#infile = open('/data/datasets/rmh2017-bt/aligned_mim.sample_5m.tsv')
#ofile = open('en-is.train.is.ner', 'w')
#ofile = open('alligned_mim.sample_5m.is.ner', 'w')
#ofile = open('rest.test', 'w')
...
ranges = [(a['start'], a['end']) for a in doc.to_json()['tokens']]
