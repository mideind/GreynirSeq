...
@register_task("multiparser")
class MultiParserTask(FairseqTask):
    @staticmethod
    def add_args(parser):
        """Add task-specific arguments to the parser."""
        parser.add_argument("data", metavar="FILE", help="file prefix for data")
        # parser.add_argument("icepahc-data", metavar="FILE", help="file prefix for data")
        # parser.add_argument("mim-data", metavar="FILE", help="file prefix for data")
        parser.add_argument(
    ...
    def __init__(
        ...
        self.term_dictionary = term_dict
        ...
        self.num_term_cats = len(self.term_schema.label_categories)
        self.num_term_groups = len(self.term_schema.group_names)
        self.num_term_labels = len(self.term_schema.labels)
    ...
    def setup_task(cls, args: argparse.Namespace, **kwargs):
        ...
        term_dict, term_schema = cls.load_label_dictionary(args, args.term_schema)
        logger.info("[term] dictionary: {} types".format(len(term_dict)))

        return MultiParserTask(
            args,
            data_dict,
    ...
    def load_dataset(self, split: str, combine: bool = False, **kwargs):
        ...
        target_spans, nterm_cats = DynamicLabelledSpanDataset.make_both(
            labelled_spans, self.nterm_dictionary, seed=self.args.seed
        )

        term_targets_path = Path(self.args.data) / "{}.term".format(split)
        pos_dataset = data_utils.load_indexed_dataset(
            str(term_targets_path), self.nterm_dictionary, self.args.dataset_impl, combine=combine
        )
        assert labelled_spans is not None, "could not find terminal labels: {}".format(nterm_targets_path)
        term_cats, term_attrs = POSDataset.make_both(
            pos_dataset, self.source_dictionary, self.term_dictionary, label_sep=self.term_schema.separator
        )
        ...
        dataset = {
            ...
            "target_term_cats": RightPadDataset(term_cats, pad_idx=self.term_dictionary.pad()),
            "target_term_attrs": RightPad2dDataset(term_attrs, pad_idx=0),
            "target_term_cats_exclude": RightPadDataset(
                IgnoreLabelsDataset(
                    term_cats, [self.term_dictionary.index(c) for c in self.term_schema.ignore_categories]
                ),
                pad_idx=0,
            ),
