...
class DynamicGPT2BPEDropoutResampling(BaseWrapperDataset):
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            for part in parts:
                part_bpe_str_old = [
                    int(elem) for elem  in part.split(" ") if elem  # filter trailing whitespace
                ]
                part_bpe_str_new = " ".join(
                    map(
                        str,
                        self.hf_tokenizer.encode(
                            self.hf_tokenizer.decode(part_bpe_str_old)
                        ).ids,
                    )
                )
                hf_bpe_tokens_str.append(part_bpe_str_new)
            item = self.source_dictionary.encode_line(
                unk_sym.join(hf_bpe_tokens_str),
                add_if_not_exist=False
            ).long()

            if item_old[0] == self.source_dictionary.bos():
                item = torch.cat([
                    torch.tensor([self.source_dictionary.bos()]),
                    item
                ])
