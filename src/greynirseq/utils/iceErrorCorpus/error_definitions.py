"""
The categories and supercategories in this file are shamelessly stolen from GreynirCorrect

XXX WARNING! The iceErrorCorpus updates its definitions every so often and therefore these
lists are not comprehensive!
TODO: Make some tool that figures out what is missing and helps you fix it.
"""
...
_supercategory_reverse: Dict[str, str] = {}
_simcategory_reverse: Dict[str, str] = {}
def _create_reverse_dicts():
    global _supercategory_reverse
    global _simcategory_reverse

    if len(_supercategory_reverse) == 0:
        _supercategory_reverse = {}
        for sup in SUPERCATEGORIES:
            for cat in SUPERCATEGORIES[sup]:
                _supercategory_reverse[cat] = sup

    if len(_simcategory_reverse) == 0:
        _simcategory_reverse = {}
        for sim in SIMCATEGORIES:
            for cat in SIMCATEGORIES[sim]:
