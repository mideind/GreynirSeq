...
def simple_rules():
    global Rules
    with open(f"{LOCAL_DIR}/rules/simple_universal.txt") as f:
        for line in f:
            line = line.strip()
            if len(line) == 0:
                continue
...
def word_substitution_rules():
    global Rules
    substitutions = {}
    with open(f"{LOCAL_DIR}/rules/english_word_pairs.txt") as f:
        for line in f:
            line = line.strip()
            if len(line) == 0:
                continue
...
def char_noise():
    ...
    noise_chars = string.ascii_lowercase + string.digits + "´'°.,'"
    ...
    def add_random_char(w):
        if coinflip(1 - FLIP_CHANCE):
            ...
            if not w[i].isdigit():
                ...
                return w[:i] + noise_chars[rand_char_int] + w[i:]
        return w
    ...
    def replace_random_char(w):
        if coinflip(1 - FLIP_CHANCE):
            ...
            if not w[i].isdigit():
                rand_char_int = random() % len(noise_chars)
                return w[:i] + noise_chars[rand_char_int] + w[i + 1 :]
        return w
