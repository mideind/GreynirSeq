...
def get_min_hun_distance(words1, words2):
    ...
    for i in range(len(words1)):
        ...
        for j in range(len(words2)):
            w2 = words2[j]
            row.append(
                1 - distance.get_jaro_distance(w1, w2, winkler=True, scaling=0.1)
            )
        values.append(row)
    ...
    for i in range(len(row_ids)):
        ...
        col_id = col_ids[i]
        hits.append(
            (words1[row_id], words2[col_id], row_id, col_id, values[row_id][col_id])
        )
...
class ParallelNER:
    ...
    stats = {dataset: copy.copy(BASE_STATS) for dataset in DATASETS}
    ...
    default_mode = "is"
    provenance_sets = {path: set() for path in DATA_PATHS}
    ...
    def print_line(self, p1, p2, pair_info, file_name):
        ...
        for pair in pair_info["pair_map"]:
            ...
            alignments.append("{}-{}".format(":".join(src), ":".join(tgt)))
        ...
        if pers1 and pers2:
            ...
            if hits:
                for hit in hits:
                    ...
                    self.ner_pair_hist[
                        "{}\t{}\t{}\t{}".format(
                            text_1, text_2, text_1 == text_2, distance
                        )
                    ] += 1
        ...
        for origin in self.stats:
            ...
            print(
                tbl_num_string.format(
                    ...
                    st[
                        "number_lines_w_per_mism"
                    ],  # Actual not same count, but includes,
