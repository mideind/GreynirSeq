...
def english_ner(input_file, output_file):
    ...
    model = AutoModelForTokenClassification.from_pretrained("dbmdz/bert-large-cased-finetuned-conll03-english").to('cuda')
    ...
    def hugface_tok_ner(sequence):
        ...
        bert_tokens = [(token, label_list[prediction]) for token, prediction in zip(tokens, predictions[0].tolist())][1:-1]
